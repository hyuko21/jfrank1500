...
(function () {
  ...
  if (privateKey) {
    servidor = https.createServer({
      ...
      console.log(`Servidor HTTPS [${process.pid}] escutando na porta 8000`);
    });
  } else {
    servidor = app.listen('8000').on('listening', () => {
      console.log(`Servidor HTTP [${process.pid}] escutando na porta 8000`);
    });
  }
  process.on('exit', (err) => {
    console.log('Fechando servidor web');
    servidor.close(() => {
      console.log('Fechando conexao com o banco de dados');
      // connection.close();
    })
  }).on('SIGTERM', (err) => {
    console.log('SIGTERM');
